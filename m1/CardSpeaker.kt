 ...
 *
 * Speech goes through Edge neural TTS (cached MP3s); if synthesis fails
 * (offline, endpoint change) it falls back to the device's own TTS engine
 * so studying never hard-stops.
 */
class CardSpeaker(private val context: Context, private val scope: CoroutineScope) {
    ...
    private suspend fun speakText(text: String) {
        val file: File? = try {
            withContext(Dispatchers.IO) { EdgeTts.synthesize(context, text) }
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
